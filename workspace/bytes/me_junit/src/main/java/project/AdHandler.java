...
public class AdHandler {
    ...
    public void addNewAd(String description, String type, int sellerID)
        ...
        if (typesList.contains(type) == false) {
            String msg = "\nInvalid Ad type entered. Supported are " + this.typesList.toString();
    ...
    public void addNewMeeting(int adId, String meetingTime, String customerName) {
        ...
        LocalDateTime localDateTime =
                LocalDateTime.parse(meetingTime, DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"));
    ...
    //         return true;
    //     } else if (timeToCheck.isAfter(LocalTime.of(14, 0))
    //             && timeToCheck.isBefore(LocalTime.of(15, 0))) {
    //         return true;
    //     } else if (timeToCheck.isAfter(LocalTime.of(18, 0))
    //             && timeToCheck.isBefore(LocalTime.of(20, 0))) {
    //         return true;
    //     }
    ...
    /*
     * public void addNewAdInputLocalDateTime(String description, String type, int sellerID,
     * LocalDateTime localDateTime) throws FileNotFoundException { Seller seller =
     * sellerMap.get(sellerID);
     * 
     * // limit size of Ad description to 50 characters if (description.length() > 50) { throw new
     * RuntimeException("\nDescription length should be less thn 50"); }
     * 
     * File typesFile = new File("src/main/resources/types.txt");
     * 
     * Scanner scanner = new Scanner(typesFile); boolean isValidTypeFlag = false;
     * 
     * while(scanner.hasNext()) { String typeRead = scanner.next().trim(); if
     * (typeRead.equals(type)) { isValidTypeFlag = true; break; } } scanner.close();
     * 
     * if (isValidTypeFlag == false) { String msg = "\nInvalid Ad type entered. Supported are " +
     * this.typesList.toString(); throw new RuntimeException(msg); }
     * 
     * // System time used AdData adData = new AdData(description, type, seller, localDateTime);
     * 
     * this.allAdsList.add(adData); }
     */
