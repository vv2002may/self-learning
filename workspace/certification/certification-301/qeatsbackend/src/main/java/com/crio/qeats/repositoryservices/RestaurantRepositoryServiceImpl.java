...
public class RestaurantRepositoryServiceImpl implements RestaurantRepositoryService {
  ...
  public List<Restaurant> findAllRestaurantsCloseBy(Double latitude, Double longitude,
      LocalTime currentTime, Double servingRadiusInKms) {
        if (latitude == null || longitude == null || currentTime == null || servingRadiusInKms == null || servingRadiusInKms <= 0) {
          return Collections.emptyList(); // Return empty list if validation fails
      }
  
      // Check if restaurantRepository is not null
      if (restaurantRepository == null) {
          throw new IllegalStateException("restaurantRepository is not initialized");
      }
  
      // Fetch all restaurants using the restaurant repository
      List<RestaurantEntity> allRestaurants = restaurantRepository.findAll();
  
      // Ensure allRestaurants is not null before filtering
      if (allRestaurants == null) {
          return Collections.emptyList(); // Handle case where no restaurants are found
      }
  
      // Filter and map the restaurants
      List<Restaurant> restaurants = allRestaurants.stream()
          .filter(restaurantEntity -> restaurantEntity != null && 
