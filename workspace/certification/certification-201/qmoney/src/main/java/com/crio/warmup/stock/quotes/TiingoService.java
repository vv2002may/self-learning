...
public class TiingoService implements StockQuotesService {
  ...
  private String token="f1f0c3563894d72598e398ab2fee37ed9d58c66a";
  ...
  public List<Candle> getStockQuote(String symbol, LocalDate from, LocalDate to) throws Exception    {
    ...
    try {
      ...
      candleList=Arrays.asList(om.readValue(result, TiingoCandle[].class));
    } catch (JsonMappingException e) {
      e.printStackTrace();
      throw new StockQuoteServiceException("Error from TiingoService : cannot process json",e);
...
//     if (to.isBefore(from)) {
//         throw new RuntimeException("Invalid date range: 'to' date is before 'from' date.");
//     }

//     TiingoCandle[] candles;
//     try {
//         // Fetch the data directly into an array of TiingoCandle
//         candles = restTemplate.getForObject(buildUri(symbol, from, to), TiingoCandle[].class);
//     } catch (Exception e) {
//         throw new StockQuoteServiceException("Error fetching stock quotes from TiingoService");
//     }

//     // Ensure that we return an empty list if candles is null or empty
